...
export default function Summary({ goImport }: { goImport: () => void }) {
  ...
  const totalAnim = Math.round(useCountUp(s.total));
  const placesAnim = Math.round(useCountUp(s.places));
  ...
  return (
    <div style={{ padding: "24px 32px 40px" }}>
      <div className="fv-rise-up" style={{ ...rise(0), display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 16 }}>
        <Metric label="照片 + 视频" value={String(totalAnim)} sub={`${s.images} 图 · ${s.videos} 视频`} accent />
        <Metric label="时间跨度" value={s.from ? `${new Date(s.from).getFullYear()}–${new Date(s.to).getFullYear()}` : "—"} sub={s.from ? `${dateLabel(s.from)} 起` : ""} />
        <Metric label="地点" value={String(placesAnim)} sub={`${s.withGps} 张含定位`} />
        <Metric label="收藏 / 私密" value={`${s.favorites} / ${s.privateCount}`} sub="收藏 / 私密区" />
      </div>

      <div className="fv-rise-up" style={{ ...rise(90), ...card, padding: "18px 22px" }}>
          ...
          {s.years.map(([y, c], i) => (
            <div key={y} style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1, height: 14, background: "var(--track)", borderRadius: 7, overflow: "hidden" }}>
                <div className="fv-barx" style={{ animationDelay: `${i * 50}ms`, height: "100%", width: `${(c / s.maxYear) * 100}%`, background: "linear-gradient(90deg, var(--accent), #5E5CE6)", borderRadius: 7 }} />
              </div>
...
function Metric({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div style={{ position: "relative", overflow: "hidden", ...(accent ? { background: "linear-gradient(155deg, var(--accent), #5E5CE6)", color: "#fff" } : card), borderRadius: 14, padding: "16px 20px", ...(accent ? { boxShadow: "0 8px 20px -6px var(--accent-soft)" } : {}) }}>
      {accent && <div aria-hidden className="fv-sheen" />}
