...
import type { DevSnippet, DevWorld } from "../../vault/types";
...
export default function Snippets({ dw, mut }: { dw: DevWorld; mut: Mut }) {
  ...
  const items = q.trim()
    ? dw.snippets.filter((s) => (s.title + s.code + (s.lang ?? "") + (s.tags?.join(" ") ?? "")).toLowerCase().includes(q.trim().toLowerCase()))
    : dw.snippets;

  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center" }}>
        <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, height: 34, padding: "0 12px", borderRadius: 9, background: "var(--fill-quaternary)", border: "0.5px solid var(--separator)" }}>
          <IconSearch />
          ...
          {items.map((s) => (
            <div key={s.id} className="fv-rise" style={{ ...card, padding: "12px 14px", overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
                ...
                {s.lang && <span style={{ fontSize: 10.5, fontWeight: 600, color: "var(--accent)", background: "var(--accent-soft)", padding: "1px 7px", borderRadius: 5, flex: "none" }}>{s.lang}</span>}
              ...
              <pre className="fv-scroll" style={{ margin: 0, padding: "10px 12px", borderRadius: 8, background: "var(--fill-quaternary)", border: "0.5px solid var(--separator)", fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 12, lineHeight: 1.6, color: "var(--text-primary)", overflow: "auto", maxHeight: 180, whiteSpace: "pre" }}>{s.code}</pre>
