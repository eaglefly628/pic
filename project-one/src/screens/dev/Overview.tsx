...
const dueToday = (t: DevTask) => isOpen(t) && t.due && fmtDue(t.due).tone !== "none" && fmtDue(t.due).tone !== "soon";

export default function Overview({ dw, mut, userName, goto }: { dw: DevWorld; mut: Mut; userName: string; goto: (t: Tab) => void }) {
  const [quick, setQuick] = useState("");
  const today = new Date().toLocaleDateString("zh-CN", { month: "long", day: "numeric", weekday: "long" });
  const openTasks = dw.tasks.filter(isOpen);
  const todayCount = dw.tasks.filter(dueToday).length;
  const inProgress = [...dw.tasks.filter((t) => t.status === "doing"), ...dw.tasks.filter((t) => t.status === "todo")]
    .sort((a, b) => (a.due ? Date.parse(a.due) : Infinity) - (b.due ? Date.parse(b.due) : Infinity)).slice(0, 5);
  const recentNotes = [...dw.notes].sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 4);
  ...
  const stats: { icon: React.ReactNode; label: string; value: number; tab: Tab; tone: string }[] = [
    ...
    { icon: <IconNote size={18} stroke="var(--green)" />, label: "笔记", value: dw.notes.length, tab: "notes", tone: "var(--green)" },
    { icon: <IconCode size={18} stroke="#5E5CE6" />, label: "代码片段", value: dw.snippets.length, tab: "snippets", tone: "#5E5CE6" },
  ];

  return (
    <div>
      <div className="fv-rise" style={{ ...rise(0), marginBottom: 18 }}>
        <div style={{ fontSize: 20, fontWeight: 600, color: "var(--text-primary)", letterSpacing: "-0.01em" }}>👨‍💻 {greeting()}，{userName}</div>
      ...
      <div className="fv-rise" style={{ ...rise(120), display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 18 }}>
        {stats.map((s) => (
          <button key={s.label} onClick={() => goto(s.tab)} className="fv-press" style={{ ...card, padding: "16px 18px", textAlign: "left", border: "none", cursor: "pointer" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ width: 34, height: 34, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", background: `color-mix(in srgb, ${s.tone} 14%, transparent)` }}>{s.icon}</span>
            ...
            {inProgress.map((t) => {
              ...
              return (
                <div key={t.id} className="fv-row" style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 8px", borderRadius: 8 }}>
                  ...
                  {t.status === "doing" && <span style={{ fontSize: 10.5, fontWeight: 600, color: "var(--accent)", background: "var(--accent-soft)", padding: "1px 7px", borderRadius: 5, flex: "none" }}>进行中</span>}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ ...card, padding: "16px 18px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {dw.links.slice(0, 6).map((l) => (
                <a key={l.id} href={l.url} target="_blank" rel="noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, color: "var(--text-secondary)", background: "var(--fill-quaternary)", border: "0.5px solid var(--separator)", padding: "5px 10px", borderRadius: 8, textDecoration: "none" }}><IconLink size={12} stroke="currentColor" />{l.title}</a>
              ))}
              {dw.links.length === 0 && <div style={{ fontSize: 12.5, color: "var(--text-tertiary)" }}>暂无书签</div>}
            </div>
          </div>
        </div>
      </div>
