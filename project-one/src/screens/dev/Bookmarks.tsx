...
import type { DevLink, DevWorld } from "../../vault/types";
...
export default function Bookmarks({ dw, mut }: { dw: DevWorld; mut: Mut }) {
  ...
  for (const l of dw.links) { const k = l.category || "未分类"; (groups.get(k) ?? groups.set(k, []).get(k)!).push(l); }
