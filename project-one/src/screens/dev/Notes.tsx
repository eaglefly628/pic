...
export default function Notes({ dw, mut }: { dw: DevWorld; mut: Mut }) {
  const list = [...dw.notes].sort(sortNotes);
  ...
  useEffect(() => {
    const n = dw.notes.find((x) => x.id === sel) ?? null;
    setDraft(n);
    ...
    <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 14, height: 600 }}>
      ...
      <div style={{ ...card, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: 10, display: "flex", gap: 8, borderBottom: "0.5px solid var(--separator)" }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, height: 32, padding: "0 10px", borderRadius: 8, background: "var(--fill-quaternary)", border: "0.5px solid var(--separator)" }}>
            <IconSearch />
          ...
          <button className="fv-icnbtn" onClick={create} title="新建笔记" style={{ width: 32, height: 32, flex: "none", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8, background: "var(--accent-soft)", border: "none", cursor: "pointer", color: "var(--accent)" }}><IconPlus size={16} /></button>
        </div>
        <div className="fv-scroll" style={{ flex: 1, overflowY: "auto" }}>
          {filtered.map((n) => (
            <button key={n.id} onClick={() => setSel(n.id)} className="fv-row" style={{ display: "block", width: "100%", textAlign: "left", border: "none", borderBottom: "0.5px solid var(--separator)", cursor: "pointer", padding: "10px 12px", background: sel === n.id ? "var(--hover)" : "transparent" }}>
          ...
          <>
            <div style={{ padding: "12px 16px 10px", borderBottom: "0.5px solid var(--separator)" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center" }}>
                ...
                <button className="fv-tap" onClick={() => patch({ pinned: !draft.pinned })} title="置顶" style={{ display: "inline-flex", alignItems: "center", gap: 4, height: 30, padding: "0 10px", borderRadius: 8, border: "0.5px solid var(--separator)", cursor: "pointer", background: draft.pinned ? "var(--accent-soft)" : "var(--fill-quaternary)", color: draft.pinned ? "var(--accent)" : "var(--text-secondary)", fontSize: 12, fontWeight: 600 }}><IconPin size={13} stroke="currentColor" />{draft.pinned ? "已置顶" : "置顶"}</button>
                <button className="fv-icnbtn" onClick={del} title="删除笔记" style={{ width: 30, height: 30, flex: "none", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8, background: "var(--fill-quaternary)", border: "0.5px solid var(--separator)", cursor: "pointer", color: "var(--red)" }}><IconTrash size={14} stroke="currentColor" /></button>
              </div>
            </div>
